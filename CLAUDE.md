...
- 料金データは `index.html` 内の `PRICING_DATA`(JSON)で手動管理
...
- `index.html` — サイト本体(HTML/CSS/JS/料金データすべてここ)
- `ogp.png` — OGP画像
...
4. 本物だった場合:
   - Web検索等で公式情報を裏取りする
   - `index.html` の `PRICING_DATA` の更新案を提示する(勝手にコミットせず、まず提案)
...
- `tier` は各社ラインナップ内の位置づけで付ける:
  `"flagship"`(最上位) / `"standard"`(中位・日常用) / `"light"`(軽量・低価格)
- 新モデル追加時は `tier` を必ず付ける

## 監視対象URLについて

- `watch_prices.py` の `TARGETS` と `index.html` の各 `pricing_url` は揃えておく
...
- サイトは静的1ファイル構成を維持する。ビルドツールやフレームワークは導入しない
